...
}b.init=function(){var e=["3rd","libs"];
b.js=[];
...
UltimateLib.Compatibility=(function(c){c.init=function(){};
function b(){var d=Character.prototype._doTraining;
var e=Character.prototype._doResearch;
Character.prototype._doTraining=function(h,f){if(GameManager.company.gameLog.length===0){var g={releaseWeek:GameManager.company.currentWeek};
GameManager.company.gameLog.push(g);
d(h,f);
GameManager.company.gameLog=[];
}if(GameManager.company.gameLog.length>=1){d(h,f);
}};
...
UltimateLib.Contracts=(function(e){UltimateLib.Logger.log("UltimateLib.Contracts loading...");
e.init=function(){c();
...
e.Medium=[];
e.Large=[];
e.addContract=function(f){if(!a(f)){UltimateLib.Logger.log("Contract Failed Compatiblity Check: "+f.name);
console.log("Failed");
return;
}var g=b(f);
switch(g.size){case"small":e.Small.push(g);
break;
case"medium":e.Medium.push(g);
break;
case"large":e.Large.push(g);
break;
default:break;
}UltimateLib.Logger.log("Contract Added: "+g.name);
};
function a(f){return(Checks.checkPropertiesPresent(f,["name","description","requiredD","requiredT","payment","penalty","weeksToFinish","rF","size"]));
}function b(g){var f=g;
return{id:"genericContracts",name:f.name,description:f.description,isRandom:f.isRandom,canTrigger:f.canTrigger,requiredD:f.requiredD,requiredT:f.requiredT,spawnedD:0,spawnedT:0,payment:f.payment,penalty:f.penalty,weeksToFinish:f.weeksToFinish,rF:f.rF,isGeneric:true,size:f.size};
}function c(){var f=ProjectContracts.getAvailable;
ProjectContracts.getAvailable=function(g,i){var h=f(g,i);
if(i=="generic"){h.addRange(UltimateLib.Contracts.collection(g));
UltimateLib.Logger.log("Contract Collection Added");
}return h;
};
}function d(g,f){if(g===true){if(!(1==Math.floor((Math.random()*f)+1))){g=false;
}}if(g===false){g=true;
}return g;
}e.collection=function(g){var f=[];
f.addRange(e.Small);
UltimateLib.Logger.log("Small Contracts Added");
if(g.flags.mediumContractsEnabled){f.addRange(e.Medium);
UltimateLib.Logger.log("Medium Contracts Added");
}if(g.flags.largeContractsEnabled){f.addRange(e.Large);
UltimateLib.Logger.log("Large Contracts Added");
}f=$.grep(f,function(h,j){return h.canTrigger(g)&&d(h.isRandom,h.randomChance);
});
return f;
};
...
UltimateLib.Publishers=(function(b){UltimateLib.Logger.log("UltimateLib.Publishers loading...");
b.init=function(){};
b.Names=[];
b.addPublisherName=function(){if(arguments[0]===undefined){return;
}for(var c=0;
c<arguments.length;
c++){b.Names.push(arguments[c]);
}};
function a(){var c=ProjectContracts.getAvailable;
ProjectContracts.getAvailable=function(d,f){var e=c(d,f);
if(f=="gameContract"){publishers.addRange(UltimateLib.Publisher.collection());
UltimateLib.Logger.log("Publisher Collection Added");
}return e;
};
}b.collection=function(){var c=GDT.getDataStore("UltimateLib").settings;
return c.publisherNames;
};
UltimateLib.Logger.log("UltimateLib.Publishers loaded :-)");
return b;
})(UltimateLib.Publishers||{});
UltimateLib.Research=(function(b){UltimateLib.Logger.log("UltimateLib.Research loading...");
b.init=function(){UltimateLib.Logger.log("UltimateLib.Research init ran.");
};
b.addSpecial=function(d){if(!c(d)){UltimateLib.Logger.log("Special Research Failed Compatiblity Check. "+d);
return;
}Research.SpecialItems.push(d);
UltimateLib.Logger.log("Special Research Added");
};
b.addSpecialResearch=function(d){if(!c(d)){UltimateLib.Logger.log("Special Research Failed Compatiblity Check. "+d);
return;
}Research.SpecialItems.push(d);
UltimateLib.Logger.log("Special Research Added");
};
b.addEngineResearch=function(d){GDT.addResearchItem(d);
UltimateLib.Logger.log("Engine Research Added");
};
function c(d){return(Checks.checkPropertiesPresent(d,["id","name","pointsCost","duration","cost","category","categoryDisplayName"])&&Checks.checkUniqueness(d,"id",Research.getAllItems()));
}b.addLab=function(d){if(!a(d)){UltimateLib.Logger.log("Lab Research Failed Compatiblity Check. "+d);
return;
}Research.bigProjects.push(d);
UltimateLib.Logger.log("Lab Research Added");
};
b.addRndResearch=function(d){if(!a(d)&&d.targetZone===2){UltimateLib.Logger.log("RnD Lab Research Failed Compatiblity Check. "+d);
return;
}Research.bigProjects.push(d);
UltimateLib.Logger.log("RnD Lab Research Added");
};
b.addHardwareResearch=function(d){if(!a(d)&&d.targetZone===0){UltimateLib.Logger.log("Hardware Research Failed Compatiblity Check. "+d);
return;
}Research.bigProjects.push(d);
UltimateLib.Logger.log("Hardware Research Research Added");
};
function a(d){if(!(Checks.checkPropertiesPresent(d,["id","name","pointsCost","iconUri","description"])&&Checks.checkUniqueness(d,"id",Research.getAllItems()))){return false;
}return true;
...
a.init=function(){UltimateLib.Configuration.addTab("visualsConfig","Visuals","");
a.Tweaks.init();
};
...
console.log(a);
a.Tweaks=(function(c){console.log(c);
...
g.append(".orangeButton, .deleteButton, .whiteButton, .selectorButton, .baseButton, .contextMenuButton, .ul-vt-button { border-radius: "+f+"px; }");
...
g.append(".featureProgressContainer, .staffDTBarContainer, .rsNavItem, .rsThumb, .projectStatusCard, .selectableGameFeatureItem, .ul-vt-bar { border-radius: "+f+"px; }");
g.append(".featurePreview1, .featureProgress, .featureProgressGain, .ul-vt-bar-left { border-top-left-radius: "+f+"px; border-bottom-left-radius: "+f+"px }");
...
c.setAllTweaks=function(f){console.log(c);
c.setRoundedWindows();
c.setScrollBar();
c.setRoundedButtons();
c.setRoundedBars();
