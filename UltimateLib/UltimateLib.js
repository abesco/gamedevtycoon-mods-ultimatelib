...
function b(){var e=LevelCalculator.getFeatureLevel;
LevelCalculator.getFeatureLevel=function(f,h){var i;
try{i=e(f,h);
}catch(g){i=0;
}return i;
};
...
function a(k,n){try{var m=GDT.getDataStore("UL-Research").data[k.id].complete;
}catch(f){GDT.getDataStore("UL-Research").data[k.id]={complete:false};
}if(n===0){var g=k.canResearch;
k.canResearch=function(e){return e.flags.hwLabUnlocked===true&&g(e);
};
}if(k.repeatable===false){var h=k.complete;
var i=k.canResearch;
k.complete=function(e){GDT.getDataStore("UL-Research").data[k.id].complete=true;
h(e);
};
k.canResearch=function(e){return GDT.getDataStore("UL-Research").data[k.id].complete!==true&&i(e);
};
}if(k.repeatable===true){var j=k.complete;
k.complete=function(e){GDT.getDataStore("UL-Research").data[k.id].complete=true;
j(e);
};
}if(!(k.iconUri.length>0)){k.iconUri="./mods/UltimateLib/img/defaultResearch.png";
}var l={id:"UL-Research-"+k.id,name:k.name,pointsCost:k.pointsCost,canResearch:k.canResearch,iconUri:k.iconUri,description:k.description,targetZone:n,complete:k.complete};
return l;
