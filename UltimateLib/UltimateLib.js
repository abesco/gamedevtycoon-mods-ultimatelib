...
e.hidden=d.hidden;
e.canEarnMultiple=d.canEarnMultiple;
e.toString=function(){return"UltimateLib.Achievements "+e.title+": "+e.description;
};
return e;
};
b.collection=[];
b.add=function(d){if(d instanceof UltimateLib.Achievements.create){d.id="UltimateLibAchievment."+d.id;
b.collection.push(d);
}};
...
function a(j,m){try{var l=GDT.getDataStore("UL-Research").data[j.id].complete;
}catch(f){GDT.getDataStore("UL-Research").data[j.id]={complete:false};
}if(j.repeatable===false){var g=j.complete;
var h=j.canResearch;
j.complete=function(e){GDT.getDataStore("UL-Research").data[j.id].complete=true;
g(e);
};
j.canResearch=function(e){return GDT.getDataStore("UL-Research").data[j.id].complete!==true&&h(e);
};
}if(j.repeatable===true){var i=j.complete;
j.complete=function(e){GDT.getDataStore("UL-Research").data[j.id].complete=true;
i(e);
};
}if(!(j.iconUri.length>0)){j.iconUri="./mods/UltimateLib/img/defaultResearch.png";
}var k={id:"UL-Research-"+j.id,name:j.name,pointsCost:j.pointsCost,canResearch:j.canResearch,iconUri:j.iconUri,description:j.description,targetZone:m,complete:j.complete};
return k;
